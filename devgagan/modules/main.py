...
from config import API_ID, API_HASH, FREEMIUM_LIMIT, PREMIUM_LIMIT, OWNER_ID, STRING
...
async def single_link(_, message):
    ...
    try:
        ...
        if data and data.get("session"):
            ...
            try:
                device = 'Vivo Y20'
                userbot = Client(":userbot:", api_id=API_ID, api_hash=API_HASH, device_model=device, session_string=STRING)
                await userbot.start()                
            except:
                users_loop[user_id] = False
